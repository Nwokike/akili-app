...
class AIService:
            ...
            try:
                ...
                if resp.status_code != 200:
                    try:
                        ...
                        print(f"\n[AI Error Detail - Attempt {attempt+1}] {json.dumps(error_detail, indent=2)}", flush=True)
                    except:
                ...
                async with self._client.stream("POST", f"{API_GATEWAY}/chat", json=payload) as response:
                    if response.status_code != 200:
                        ...
                        try:
                            ...
                            print(f"\n[AI Stream Error Detail - Attempt {attempt+1}] {json.dumps(error_detail, indent=2)}", flush=True)
                        except:
    ...
    async def chat_stream(
        ...
        base_prompt = get_dynamic_system_prompt(
            user_name=state.user_name or "Student",
            education_level=state.education_level or "Grade 10"
        )
        if system_prompt:
        ...
        for iteration in range(MAX_TOOL_ITERATIONS):
            ...
            if not is_tool_call:
                print(f"[AI] <<< Loop Finished (No more tools requested).", flush=True)
                break 
            ...
            for _, tc in tool_calls_buffer.items():
                ...
                if func_name == "search_web":
                    ...
                    result_content = str(res)
                    print(f"[AI Tool Result] read_page: {len(result_content)} chars. Snippet: {result_content[:300]}...", flush=True)
        ...
        **kwargs
        ...
        for iteration in range(MAX_TOOL_ITERATIONS):
            ...
            if not message.get("tool_calls"):
                print(f"[AI] <<< Loop Finished (Final Answer Generated).", flush=True)
