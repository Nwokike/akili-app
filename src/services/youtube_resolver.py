...
def parse_decipher_algo(js_code: str) -> DecipherAlgorithm:
    ...
    fwd_h = {i: v for i, v in enumerate(h_table)}
    ...
    try:
        ...
        helper_def_pattern = re.compile(
            rf'(?:var|const|let)\s+{helper_name}\s*=\s*\{{(.*?)\}};',
            re.DOTALL
        )
        ...
        for name, args, body in helper_fn_pattern.findall(def_body):
        ...
        for helper_obj, method_xor, arg_xor, arg_literal in statements:
            ...
            if arg_xor:
                arg_val = key_val ^ int(arg_xor)
            else:
                arg_val = int(arg_literal)
