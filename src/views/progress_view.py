...
async def build_progress_view(page: ft.Page, navigate) -> ft.View:
    ...
    level_card = ft.Container(
        content=ft.Column([
            ...
            ft.ProgressBar(value=progress, color=AppColors.PRIMARY, bgcolor=ft.Colors.SURFACE_CONTAINER, height=8, border_radius=4),
        ], spacing=10),
        padding=24,
        border_radius=AppStyles.RADIUS,
        bgcolor=ft.Colors.SURFACE_CONTAINER_HIGHEST,
    ...
    quiz_card = ft.Container(
        content=ft.Row([
            ft.Column([
                ft.Text(f"{int(avg)}%", size=32, weight=ft.FontWeight.BOLD, color=AppColors.SUCCESS if avg >= 60 else AppColors.ERROR),
